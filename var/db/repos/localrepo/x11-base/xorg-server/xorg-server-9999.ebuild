...
EGIT_REPO_URI="https://gitlab.com/rk3588_linux/linux/xserver.git"
EGIT_BRANCH="rockchip-1.20.11-debian"
...
IUSE="${IUSE_SERVERS} debug +elogind minimal selinux suid systemd test +udev xcsecurity"
RESTRICT="!test? ( test )"
...
"
PDEPEND="
	xorg? ( >=x11-base/xorg-drivers-21.1-r2 )"
...
PATCHES=(
	...
	"${FILESDIR}"/rockchip-xorg-server-fix.patch
)
...
src_install() {
	...
	rm -f "${ED}"/usr/share/man/man1/cvt.*
	rm -f "${ED}"/usr/bin/cvt
	ln -s "${ED}"/usr/bin/Xorg "${ED}"/usr/bin/X
}
